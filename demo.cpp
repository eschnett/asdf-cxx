...
int main(int argc, char **argv) {
  ...
  auto array3d =
      make_shared<ndarray>(vector<complex128_t>{1, -2, 3i, -4i, 5 + 1i, 6 - 1i},
                           block_format_t::block, optional<vector<bool>>(),
                           vector<int64_t>{1, 2, 3});
  auto col3 = make_shared<column>("delta", array3d, optional<string>());
  auto array8d = make_shared<ndarray>(
      vector<bool8_t>{true}, block_format_t::block, optional<vector<bool>>(),
      vector<int64_t>{1, 1, 1, 1, 1, 1, 1, 1});
  auto col8 = make_shared<column>("epsilon", array8d, optional<string>());
